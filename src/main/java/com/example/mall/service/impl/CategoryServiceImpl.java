...
import com.example.mall.controller.vo.SecondLevelCategoryVO;
import com.example.mall.controller.vo.ThirdLevelCategoryVO;
...
public class CategoryServiceImpl implements CategoryService {
    ...
    public String saveCategory(GoodsCategory goodsCategory) {
        ...
        if (goodsCategoryMapper.insertSelective(goodsCategory) > 0) {
            this.updatePathTraceAndLevel(goodsCategory.getCategoryId());
    ...
    public List<IndexCategoryVO> getCategoriesForIndex() {
        List<IndexCategoryVO> mallIndexCategoryVOS = new ArrayList<>();
        //获取一级分类的固定数量的数据
    ...
    public SearchPageCategoryVO getCategoriesForSearch(Long categoryId) {
        ...
        if (thirdLevelGoodsCategory != null && thirdLevelGoodsCategory.getCategoryLevel() == CategoryLevelEnum.LEVEL_THREE.getLevel()) {
            ...
            if (secondLevelGoodsCategory != null && secondLevelGoodsCategory.getCategoryLevel() == CategoryLevelEnum.LEVEL_TWO.getLevel()) {
                ...
                searchPageCategoryVO.setSecondLevelCategoryId(secondLevelGoodsCategory.getCategoryId());
    ...
    @Override
    public List<GoodsCategory> getGoodsCategoryWithGoods() {
        List<GoodsCategory> goodsCategoryList = goodsCategoryMapper.getGoodsCategoryWithGoods();
        return goodsCategoryList;
    }

    @Override
    public List<GoodsCategory> findAllLevel1() {
        return goodsCategoryMapper.findAllLevel1();
    }


    @Override
    public void updatePathTraceAndLevel(Long categoryId) {
        try {
            ...
            if (category.getParentId() == 0 || category.getParentId() == null) {
                ...
                category.setPathTrace("/" + category.getCategoryId() + "/");
            } else {
                GoodsCategory parentCategory = this.goodsCategoryMapper.selectByPrimaryKey(category.getParentId());
                if (parentCategory != null && parentCategory.getCategoryLevel() != null) {
                    category.setCategoryLevel((parentCategory.getCategoryLevel() + 1));
                    category.setPathTrace(parentCategory.getPathTrace() + category.getCategoryId() + "/");
                }
            }
